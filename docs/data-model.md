...
- приоритет снимка участия: ЯВНОЕ значение Excel > карточка Student
  (явно выбранная или единственный точный кандидат — используется для
  автозаполнения пустых пол/институт/группа/курс в предпросмотре) > пусто;
...
- кандидаты — только ПРЕДЛОЖЕНИЯ (те же правила, что в сопоставлении:
  ...
  карточки; без fuzzy/ё-е/транслитерации); строка «готова к добавлению»,
  когда карточка определена (единственный точный кандидат ИЛИ явный выбор
  админа — в т.ч. у полных тёзок), валидация прошла и нет не разобранной
  строки-дубля; участие создаётся только явным подтверждением;
  тёзки (≥2) без выбора, 0 кандидатов и полностью одинаковые строки файла
  (равенство всего снимка, casefold) → «требуют решения» — выбрать карточку
  поиском (подстрока по ФИО/псевдониму активных карточек, GET-параметры
  `find`/`q` предпросмотра, только чтение), создать карточку+участие
  (admin), править строку, пропустить; правка ФИО строки сбрасывает явный
  выбор карточки (устаревшая связь не переносится молча — карточку
  выбирают заново); дубли блокируют добавление, только пока ОБЕ строки
  группы не разобраны — после пропуска/добавления одной вторая становится
  добавляемой («оставить одну» и «создать обе» достижимы из UI);
...
- массовое добавление «готовых» строк — одной транзакцией
  ...
  строк, строки решений в батч не попадают; если состав
  кандидатов изменился к моменту клика — весь батч отменяется;
- подтверждённые участия получают `student_ref_id` выбранной/созданной
  карточки; аудит — `event_participant_imported` на каждое участие,
  `event_participants_import_completed` при завершении.
...
- `competitions.student_ref_id` и `users.student_ref_id` — nullable-колонки
  ...
  НОВЫЕ записи получают `student_ref_id` при явном выборе/создании
  карточки: ручное добавление участника события (выбор в подсказке ФИО) и
  импорт участников события — это НЕ автоматический матчинг, решение
  всегда принимает человек. Привязка меняет ТОЛЬКО `student_ref_id`:
