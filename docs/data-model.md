...
- Excel с обязательной колонкой только «ФИО»; опциональны
  ...
  `show_in_template` (лишние колонки игнорируются; пустой шаблон
...
- кандидаты — только ПРЕДЛОЖЕНИЯ (те же правила, что в сопоставлении:
  ...
  (в т.ч. по номеру), создать карточку+участие (admin), править строку,
  пропустить; дубли блокируют добавление, только пока ОБЕ строки группы
  не разобраны — после пропуска/добавления одной вторая становится
