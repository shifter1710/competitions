...
студентов из Excel — но ВСЕ рабочие workflow по-прежнему работают только
...
импорт из Excel (Phase 2.5, см. ниже), правка, деактивация (физического
удаления студентов нет), псевдонимы ФИО. Правка карточки меняет ТОЛЬКО
актуальные данные — исторические записи о соревнованиях, отчёты и выгрузки
