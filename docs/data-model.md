...
| `discipline` | TEXT NULL | дисциплина участия (Event Model, Wave 1 P0/P1 → P5a): заполняется Excel-импортом реестра (опциональная колонка «Дисциплина») и решениями очереди конфликтов, входит в полный ключ дубля (нормализованная), отдаётся фиксированной колонкой выгрузки. Существующие строки — NULL |
| `result` | TEXT NULL | результат участия (Event Model, Wave 1 P0/P1 → P5a): отдаётся фиксированной колонкой выгрузки «Результат»; импортом реестра не пишется (значение файла идёт только в кастом-поле с тем же label, если активно) |
...
- `POST /calendar/<id>/participants` — добавление участника события:
  `approved` (см. `calendar_events`); при явном выборе карточки студента
  в подсказке ФИО запись получает `student_ref_id`;
...
- `event_participant_imported` `{event_id, student_ref_id}` — каждое участие,
  ...
  несёт `source: event-participant-import`; `event_participants_import_completed`
  `{event_id, event_name, counters}` — итоги импорта при завершении
  (содержимое файла в аудит не пишется);
...
(`position = 0`, «ждёт результата»). Подсказки ФИО при вводе участника —
...
- Excel с обязательной колонкой только «ФИО»; опциональны
  «Пол»/«Институт»/«Группа»/«Курс»/«Место» + активные кастомные поля с
  `show_in_template`, кроме url-полей (ссылки уровня записи/события к
  снимку участия не относятся; сами url-поля в реестре/обычном
  импорте-экспорте остаются — колонка url-поля в файле участников
  игнорируется как неизвестная; лишние колонки игнорируются; пустой шаблон
  скачивается; лимит 5000 строк). Название/даты/уровень/вид спорта
  в файле НЕ нужны — пресет берётся из события;
...
- кандидаты — только ПРЕДЛОЖЕНИЯ (те же правила, что в сопоставлении:
  ...
  импорт НЕ блокирует: «Готова к добавлению» — ЛЮБАЯ строка без ошибок
  валидации и без не разобранной строки-дубля, участие создаётся только
  явным подтверждением; единственный точный кандидат подбирается
  ...
  связи); тёзки (≥2) и 0 кандидатов не подбираются автоматически — строка
  всё равно «готова», участие добавляется с `student_ref_id IS NULL` и
  сопоставляется позже существующим механизмом
  (`/admin/people/reconcile`); «Требуют решения» — ТОЛЬКО полностью
  одинаковые строки файла (равенство всего снимка, casefold) и только
  пока ОБЕ строки группы не разобраны — после пропуска/добавления одной
  вторая становится добавляемой («оставить одну» и «создать обе»
  достижимы из UI); ошибочные — только ошибки валидации; действия по
  строке: выбрать карточку поиском (подстрока по ФИО/псевдониму активных
  карточек, GET-параметры `find`/`q` предпросмотра, только чтение;
  доступен и для смены уже подобранной карточки), создать карточку+участие
  (admin), править строку, пропустить; правка ФИО строки сбрасывает ОБА
  решения по карточке — явный выбор и «без карточки» (устаревшее решение
  не переносится молча, для нового ФИО кандидаты подбираются заново);
  несколько участий одного Student в событии (например, разные
  дисциплины) — нормальный случай, дубликатом не считаются; существующее
  участие показывается рядом («уже участвует…») без блокировки;
- массовое добавление «готовых» строк — одной транзакцией
  (`import_competitions`: записи + синхронизация справочников, сбой —
  откат всего); в батч входят РОВНО строки группы «Готовы к добавлению»
  на момент рендера предпросмотра — счётчик кнопки равен числу вставляемых
  строк, строки решений и ошибочные в батч не попадают; подтверждение
  кнопки предупреждает о числе строк без карточки; если эффективная
  карточка строки изменилась к моменту клика (появился/исчез тёзка,
  карточка деактивирована) — весь батч отменяется;
- подтверждённые участия получают `student_ref_id` подобранной/выбранной/
  созданной карточки или NULL («сопоставить позже»); аудит —
  `event_participant_imported` на каждое участие (`student_ref_id` может
  быть null), `event_participants_import_completed` при завершении.

Удаление события заблокировано, если по пресету уже есть совпавшие записи.
...
- `competitions.student_ref_id` и `users.student_ref_id` — nullable-колонки
  ...
  Привязка меняет ТОЛЬКО `student_ref_id`:
  снимки данных в записях, легаси-ключ `student_id`, кабинет атлета,
  отчёты, импорт/экспорт, календарь и вложения её не читают (Phase 3 —
  перевод runtime-идентичности на `student_ref_id` — НЕ начата);
