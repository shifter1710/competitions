...
- **Реестр записей** (главная страница, «Реестр» в верхнем меню): правка
  прямо в ячейках (Enter — сохранить, Esc — отмена), «+ Пустая строка» для
  быстрого ввода, сортировка кликом по заголовку, скрытие и восстановление
  колонок, перетаскивание колонок, выгрузка в Excel ровно того вида, который
  сейчас на экране. Настройки вида хранятся в браузере (localStorage)
  отдельно для каждого представления таблицы.
- **Ручной ввод и импорт**: форма «Добавить запись» (для атлета — с
  ...
  раздела «Импорт реестра»), пропуски дублей при импорте с итоговым
  счётчиком. Шаблон содержит опциональную колонку «Дисциплина» (файлы без
  неё импортируются как раньше; дисциплина учитывается в ключе дубля), а
  выгрузка реестра — фиксированные колонки «Дисциплина» и «Результат».
  Импорт реестра открывается кнопкой «Импорт» над реестром
  (admin/editor) и из хаба «Админ»; похожие на существующие строки уходят
  в очередь подтверждения (разбирает admin).
...
- **Справочники** (admin): значения видов спорта, институтов и уровней
  ...
  переименовать (с обновлением записей) или удалить (если на нём нет записей).
  Группу можно перенести в другой институт — исправление неверной привязки
  обновляет справочник, записи о соревнованиях и карточки студентов с той же
  парой институт+группа. Справочник — только подсказки (datalist) в формах:
  значения в существующих записях не валидируются и не перезаписываются.
- **Календарь соревнований** (admin/editor; просмотр — viewer): план на год,
  участники добавляются обычными записями реестра. При вводе ФИО участника
  подсказки ищут и по карточкам студентов (раздел «Студенты»): явный выбор
  карточки заполняет пустые пол/институт/группу/курс и записывает связь
  `student_ref_id`. Импорт участников события из Excel: обязательное значение
  только ФИО, название/даты/уровень/вид спорта берутся из события, пустые
  поля подтягиваются из карточки студента при точном совпадении (явные
  значения файла имеют приоритет, карточка при этом не меняется), место,
  дисциплина, результат и кастомные поля участия поддерживаются.
  Сопоставление с карточкой импорт не блокирует: единственный точный
  кандидат по ФИО подбирается автоматически (виден в предпросмотре, легко
  сбросить), а строки без карточки добавляются без связи и сопоставляются
  позже через раздел сопоставления. Дисциплина различает участия одного
  человека (100 м и 200 м — два участия), а повтор уже существующего
  участия не создаётся: точная копия помечается «уже существует», при
  других месте/результате предпросмотр предлагает обновить существующее
  (меняются только место и результат), повторы строк в файле
  пропускаются; сервер не позволяет добавить дубль и повторной загрузкой
  того же файла.
  Добавление — только после предпросмотра и явного подтверждения строк
  (подробнее — [docs/data-model.md](docs/data-model.md)). К событию можно
  прикрепить файл положения (PDF/JPEG/PNG до 5 МБ); admin может удалить
  участника прямо со страницы события. Несколько участий одного участника
  (например, разные дисциплины) показываются на странице события группой:
  общая строка — участник, строки «└─» — его участия; кнопка «+ участие»
  добавляет ещё одно участие (дисциплину, место, результат и поля участия)
  к той же карточке студента, в реестре каждое участие остаётся отдельной
  записью. Запись без соревнования связывается
  с событием календаря вручную (иконка-звено в строке реестра; admin/editor):
  кандидаты ищутся поиском с предпросмотром изменений полей, либо событие
  создаётся прямо со страницы записи; массовое связывание по совпадению
  названия и дат — в «Обслуживании базы» (admin, только однозначные
  совпадения).
...
- **Студенты** (admin): карточки студентов — актуальные институт, группа,
  ...
  а также полное удаление ошибочно созданной карточки без связей (только
  со страницы карточки; при привязанных записях/аккаунтах блокируется,
  записи о соревнованиях не удаляются).
  ...
  пишутся. Карточки — источник подсказок ФИО при ручном добавлении участника
  события (включая студентов без истории участий); runtime-идентичность
  (авторизация атлета, отчёты, merge) по-прежнему работает по легаси-ключу
  `sha256(ФИО)` — Phase 3 не начата.
