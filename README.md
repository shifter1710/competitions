...
- **Календарь соревнований** (admin/editor; просмотр — viewer): план на год,
  ...
  кастомные поля участия поддерживаются; добавление — только после
  предпросмотра и явного подтверждения строк (подробнее —
  [docs/data-model.md](docs/data-model.md)). К событию можно прикрепить
  файл положения (PDF/JPEG/PNG до 5 МБ); admin может удалить участника
  прямо со страницы события.
