...
- **Календарь соревнований** (admin/editor; просмотр — viewer): план на год,
  ...
  значения файла имеют приоритет, карточка при этом не меняется), место и
  кастомные поля участия поддерживаются. Сопоставление с карточкой импорт
  не блокирует: единственный точный кандидат по ФИО подбирается
  автоматически (виден в предпросмотре, легко сбросить), а строки без
  карточки добавляются без связи и сопоставляются позже через раздел
  сопоставления; «требуют решения» только одинаковые строки-дубли в файле.
